...
/* SECTION 16 BEGIN */
CREATE OR REPLACE FUNCTION Same_skills()  RETURNS TRIGGER AS 
$$
BEGIN
IF NEW.CNAME ='Apple'
THEN 
INSERT INTO personSkill (SELECT DISTINCT NEW.pid, PS.SKILL
						FROM PERSONSKILL PS JOIN WORKSFOR WF
						ON PS.PID = WF.PID AND WF.CNAME ='Google');
END IF;						
RETURN NULL;
END;
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE TRIGGER Same_skills
AFTER INSERT ON worksFor 
FOR EACH ROW EXECUTE PROCEDURE Same_skills();
...
CREATE OR REPLACE FUNCTION KNOWING() RETURNS TRIGGER AS $$
BEGIN
...
CREATE OR REPLACE TRIGGER KNOWING_TGR AFTER
UPDATE ON WORKSFOR
